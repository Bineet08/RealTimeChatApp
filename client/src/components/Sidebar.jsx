...
import { userDummyData } from '../assets/userDummyData.js'
        ...
        {userDummyData.map((user,index)=>(
          <div onClick={()=>{setSelectedUser(user)}} key = {index}
           className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id ===user._id && 'bg-[#282142]/50'} `}>
            ...
            className='w-[35] aspect-square rounded-full'/>
            ...
            {
              index>2 && <p className='absolute top-4 text-xs right-4 h-5 w-5 flex justify-center items-center rounded-full bg-violet-500/50'>{index}</p>
            }
          </div>
        ))}
      </div>
